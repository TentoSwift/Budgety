...
extension Color {
    /// シート (家計簿) のカラー選択に使う共通パレット。
    /// iOS / macOS の「新規シート」「シート編集」で同一のセットを提示するために
    /// ここに一本化する。順序も含めて全プラットフォームで共有すること。
    /// 既存シートの保存済み `colorHex` はこのパレット外の値でもそのまま有効。
    static let sheetColorPalette: [String] = [
